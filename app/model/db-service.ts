...
{
    return openDatabase({name: "$table_user", location:"default"})
}
...
export const createPatientTable = async (db: SQLiteDatabase) => 
{
    const q = 'CREATE TABLE IF NOT EXISTS table_user(user_id INTEGER PRIMARY KEY AUTOINCREMENT, firstName VARCHAR(20), lastName VARCHAR(20), gender VARCHAR(20), age INTEGER, healthInsurance VARCHAR(20), telephone VARCHAR(20))';
   await db.transaction(tx =>{
       tx.executeSql(q);
   });
}
